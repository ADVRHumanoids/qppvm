...
public:
    ...
    VectorXd GetValues() const override
    {
        ...
        for(int i : {1, 2, 3, 4})
        {
            Eigen::Vector3d F = GetVariables()->GetComponent("F" + std::to_string(i))->GetValues();
            Eigen::Vector3d p = GetVariables()->GetComponent("p" + std::to_string(i))->GetValues();
//             Eigen::Vector3d n_SE = GetVariables()->GetComponent("n" + std::to_string(i))->GetValues();
...
//             double eps = 1e-4;
            ...
            value.head<3>() +=  R.transpose()*F;
            value.tail<3>() += (p-_com).cross(R.transpose()*F);
        }
...
public:
    ...
    FrictionConstraint(const std::string& force_name) :
        ConstraintSet(5, "FrictionConstraint " + force_name),
        fname_(force_name)
    {
        mu_= 1; 

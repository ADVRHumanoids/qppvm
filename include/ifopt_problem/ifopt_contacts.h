...
//         std::cout <<"value " << value.transpose() << std::endl; 
    ...
    void FillJacobianBlock (std::string var_set, Jacobian& jac_block) const override
    {
        ...
        if(var_set == pname_)
        {
             ...
             jac_block.coeffRef(0, 0) = _P.x()*pow(_R.x(),-_P.x())*pow(t2,_P.x())*t4*t5*t8*t9*t12*(_P.x()-1.0)*1.0/pow(t5*t8*t13*t15+t9*t12*t16*t18+(_P.x()*_P.x())*pow(_R.x(),_P.x()*-2.0)*pow(t2,_P.x()*2.0)*t4,3.0/2.0)*((_C.y()*_C.y())*t16*t18*t20+(_C.z()*_C.z())*t13*t15*t19+(p.y()*p.y())*t16*t18*t20+(p.z()*p.z())*t13*t15*t19-_C.y()*p.y()*t16*t18*t20*2.0-_C.z()*p.z()*t13*t15*t19*2.0);
             ...
             t2 = _P.z()*2.0;   
             t3 = -_C.x()+p.x();
             t4 = _P.z()*_P.z();jac_block.coeffRef(0, 2) = 
             t5 = -_C.z()+p.z();   
             t6 = t2-2.0;   jac_block.coeffRef(1, 0) = 
             t7 = pow(_R.z(),-t2);   jac_block.coeffRef(1, 1) = 
             ...
             jac_block.coeffRef(1, 1) = _P.y()*pow(_R.y(),-_P.y())*t2*t5*pow(t6,_P.y())*t8*t9*t12*(_P.y()-1.0)*1.0/pow(t2*t5*t13*t15+t9*t12*t16*t18+(_P.y()*_P.y())*pow(_R.y(),_P.y()*-2.0)*pow(t6,_P.y()*2.0)*t8,3.0/2.0)*((_C.x()*_C.x())*t16*t18*t20+(_C.z()*_C.z())*t13*t15*t19+(p.x()*p.x())*t16*t18*t20+(p.z()*p.z())*t13*t15*t19-_C.x()*p.x()*t16*t18*t20*2.0-_C.z()*p.z()*t13*t15*t19*2.0);
             ...
             jac_block.coeffRef(2, 2) = _P.z()*pow(_R.z(),-_P.z())*t2*t5*t6*t9*pow(t10,_P.z())*t12*(_P.z()-1.0)*1.0/pow(t2*t5*t13*t15+t6*t9*t16*t18+(_P.z()*_P.z())*pow(_R.z(),_P.z()*-2.0)*pow(t10,_P.z()*2.0)*t12,3.0/2.0)*((_C.x()*_C.x())*t16*t18*t20+(_C.y()*_C.y())*t13*t15*t19+(p.x()*p.x())*t16*t18*t20+(p.y()*p.y())*t13*t15*t19-_C.x()*p.x()*t16*t18*t20*2.0-_C.y()*p.y()*t13*t15*t19*2.0);
